...
function Manga() {
    ...
    const slug = location.pathname.split("/")[2]; // Lấy slug từ URL
    const shareUrl = `https://demo-manga.vercel.app/manga-details/${encodeURIComponent(slug)}`;
                            ...
                            <div className={cx("information")}>
                                ...
                                <div className={cx("content")}>
                                    ...
                                    <div className={cx("action")}>
                                        ...
                                        <div onClick={() => setIsFollowOpen(true)}>
                                            <SaveIcon />
                                        </div>
                                        <div>
                                            <BellOutlineIcon />
            ...
            {mangaData ? (
                <Row justify="center" style={{ marginTop: "70px" }}>
                    ...
                    <Col span={22}>
                        ...
                        {client ? (
                            <div className={cx("vote")} onClick={() => handelVote()}>
                                {mangaData.like}
                                {isVote ? <ArrowDownOutlined /> : <ArrowUpOutlined />}
                            </div>
                        ) : (
                            <div
                                className={cx("vote")}
                        ...
                        <div className={cx("chapters")}>
                            <div className={cx("head")}>
                                <Input
                                    ...
                                    suffix={<FilterOutlined style={{ fontSize: 15 }} />}
                                />
                            </div>
                    ...
                    <Col span={22}>
                        ...
                        {client ? (
                            <div className={cx("comment-input")}>
                                <p>
                                    <i>Hãy bình luận văn minh, lịch sự và mang tính xây dựng</i>
                                </p>
                                <Input.TextArea
                                    rows={6}
                                ...
                                <div className={cx("send")}>
                                    ...
                                    <button
                                        onClick={() => createMangaComment(comment, client.user.id)}
                                    >
                                        <SendIcon />
                                    </button>
                                </div>
                            ...
                            <div className={cx("login-require")}>
                                <Link to="/login">Đăng nhập</Link> để bình luận bạn nhé
                            </div>
                        )}
                                ...
                                <div className={cx("comment")} key={comment.id}>
                                    ...
                                    <div className={cx("child")}>
                                        {comment.replies.map((reply) => (
                                            <div className={commentClassName} key={reply.id}>
                                                <Comment
                                                    comment={reply}
                                                    mangaId={mangaData.id}
                                                    commentId={comment.id}
                                        ...
                                        {comment.replies.length > 0 ? (
                                            ...
                                            >
                                                <p>{comment.replies.length} câu trả lời </p>
                                                {commentOpen ? <UpOutlined /> : <DownOutlined />}
                                            </div>
                                        ) : (
                                            ""
                                        )}
